...
export function generateRevenueTimeSeries(
  ...
  if (mode === "today") {
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      const label = dEnd.toLocaleDateString("en-US", { month: "short", day: "2-digit" });
    ...
    // "all" / "lifetime": find earliest transaction or last 12 months
    let earliestTime = now.getTime() - 365 * 24 * 3600 * 1000;
    if (succeeded.length > 0) {
      ...
      if (txTimes.length > 0) {
        ...
        if (minTx < earliestTime) earliestTime = minTx;
      }
    }
    ...
    while (cur <= now || buckets.length < 6) {
  ...
  succeeded.forEach((tx) => {
    ...
    if (targetBucket) {
      ...
      // In all-time mode, if transaction predates the first bucket, allocate to first
